...
const INTELLIGENCE_TAB_LIMIT = Math.max(0, Math.min(4, Number(process.env.BETEXPLORER_INTELLIGENCE_TAB_LIMIT || 3)));
...
async function discoverIntelligenceTabs(page) {
  return page.evaluate(() => {
    ...
    for (const anchor of [...document.querySelectorAll('a[href]')]) {
      ...
      if (!/(streak|form table|over.?under|o\/?u|ht\/?ft|half.?time.*full.?time|standings.*form)/.test(combined)) continue;
      try {
        const url = new URL(href, location.href);
        if (!(url.hostname === 'betexplorer.com' || url.hostname.endsWith('.betexplorer.com'))) continue;
        url.hash = '';
...
try {
  ...
  if (!DRY_RUN && fixtureIds.size > 0) {
    ...
    report.totals = {
      ...
      athenaShadowRuns: Number(rebuilt.body?.athenaShadowRuns || 0),
      athenaShadowPicks: Number(rebuilt.body?.athenaShadowPicks || 0),
      athenaShadowBankers: Number(rebuilt.body?.athenaShadowBankers || 0),
